...
class Gene:
    ...
    def getPdbVariantCoverageDataFrame(self, pdb):
        ...
        for vrcod, vr in self.variants.items():
            included, offset,chain = vr.includedInRange(pdb.segment_starts, pdb.segment_ends,pdb.segment_offsets,pdb.segment_chains)
            if included:                
                dic_coverage["gene"].append(self.gene)
                dic_coverage["accession"].append(self.accession)
                ...
                dic_coverage["pdb_mut"].append(vrcod[0] + str(int(vr.residue) - int(offset)) + vrcod[-1])
